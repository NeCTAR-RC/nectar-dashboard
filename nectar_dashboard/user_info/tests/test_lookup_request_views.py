...
class AdminLookupLookupTestCase(LookupListMixin, base.AdminViewTestCase):
    ...
    def test_get_q_email(self):
        response = self.client.get(self.url, {'q': self.rcs_user.email})
        self.assertStatusCode(response, 200)
        self.assertEqual(len(response.context_data['table'].get_rows()), 1)

    def test_get_q_nickname(self):
        response = self.client.get(self.url, {'q': 'doc'})
        self.assertStatusCode(response, 200)
        self.assertEqual(len(response.context_data['table'].get_rows()), 1)

    def test_get_q_unknown(self):
        response = self.client.get(self.url, {'q': "xxx@example.com"})
        self.assertStatusCode(response, 200)
        self.assertEqual(len(response.context_data['table'].get_rows()), 0)

    def test_get_e_email(self):
        response = self.client.get(self.url, {'email': self.rcs_user.email})
        self.assertStatusCode(response, 200)
        self.assertEqual(len(response.context_data['table'].get_rows()), 1)

    def test_get_e_nickname(self):
        response = self.client.get(self.url, {'email': 'doc'})
        self.assertStatusCode(response, 200)
        self.assertEqual(len(response.context_data['table'].get_rows()), 0)

    def test_get_email_unknown(self):
        response = self.client.get(self.url, {'email': "xxx@example.com"})
        self.assertStatusCode(response, 200)
        self.assertEqual(len(response.context_data['table'].get_rows()), 0)
