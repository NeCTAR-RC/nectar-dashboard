...
class AdminLookupLookupTestCase(LookupListMixin, base.AdminViewTestCase):
    ...
    def test_get_bad_verb(self):
        response = self.client.get(self.url, {'verb': 'fnord'})
        self.assertStatusCode(response, 200)
        self.assertEqual(len(response.context_data['table'].get_rows()), 0)

    def test_get_search_email(self):
        response = self.client.get(self.url, {'verb': 'search',
                                              'q': self.rcs_user.email})
        self.assertStatusCode(response, 200)
        self.assertEqual(len(response.context_data['table'].get_rows()), 1)

    def test_get_search_nickname(self):
        response = self.client.get(self.url, {'verb': 'search', 'q': 'doc'})
        self.assertStatusCode(response, 200)
        self.assertEqual(len(response.context_data['table'].get_rows()), 1)
    ...
    def test_get_lookup_email(self):
        response = self.client.get(self.url, {'verb': 'lookup',
                                              'q': self.rcs_user.email})
        self.assertStatusCode(response, 200)
        self.assertEqual(len(response.context_data['table'].get_rows()), 1)

    def test_get_lookup_nickname(self):
        response = self.client.get(self.url, {'verb': 'lookup', 'q': 'doc'})
        self.assertStatusCode(response, 200)
        self.assertEqual(len(response.context_data['table'].get_rows()), 0)

    def test_get_lookup_unknown(self):
        response = self.client.get(self.url, {'verb': 'lookup',
                                              'q': "xxx@example.com"})
        self.assertStatusCode(response, 200)
        self.assertEqual(len(response.context_data['table'].get_rows()), 0)
