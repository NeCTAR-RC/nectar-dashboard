...
class Command(BaseCommand):
    help = "Replace one Organisation with another one.  This will replace " \
    ...
    def add_arguments(self, parser):
        parser.add_argument('original',
                            help="ROR or local id of original Organisation")
        parser.add_argument('replacement',
                            help="ROR or local id of replacement Organisation")
        parser.add_argument('--disable', action="store_true",
                            help="Disable the original Organisation after "
                            "replacing it")
    ...
    def handle(self, original, replacement, **options):
        ...
        if original_org.pk == replacement_org.pk:
            print(f"Ids ({original} and {replacement}) "
        ...
        self.replace_organisations(original_org, replacement_org,
    ...
    def replace_organisations(self, original_org, replacement_org,
        ...
        for a in self.catalog.AllocationRequest.objects.filter(
            ...
            if no_dry_run:
                ...
                LOG.info(f"Changed Organisation {original_org.id} "
