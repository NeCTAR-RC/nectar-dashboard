...
class InstitutionTests(base.AllocationAPITest):
    ...
    def test_list_institutions(self):
        ...
        institution = factories.InstitutionFactory(allocation=self.allocation, name='foo')
    ...
    def test_get_institution(self):
        self.client.force_authenticate(user=self.user)
        institution = factories.InstitutionFactory(allocation=self.allocation)
        response = self.client.get('/rest_api/institutions/%s/' % institution.id)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_update_institution(self):
        self.client.force_authenticate(user=self.user)
        institution = factories.InstitutionFactory(allocation=self.allocation)
        response = self.client.patch('/rest_api/institutions/%s/' % institution.id,
                                     {'name': 'new-name'})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_delete_institution(self):
        self.client.force_authenticate(user=self.user)
        institution = factories.InstitutionFactory(allocation=self.allocation)
        response = self.client.delete('/rest_api/institutions/%s/' % institution.id)
