...
class AllocationTests(base.AllocationAPITest):
    ...
    def test_filter_contact_email(self):
        self.client.force_authenticate(user=self.admin_user)
        factories.AllocationFactory.create(contact_email='other@example.com')
        response = self.client.get('/rest_api/allocations/?contact_email=other@example.com')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(1, len(response.data['results']))
    ...
    def test_filter_allocation_home_national(self):
        self.client.force_authenticate(user=self.admin_user)
        factories.AllocationFactory.create(national=True)
        response = self.client.get('/rest_api/allocations/?allocation_home=national')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(1, len(response.data['results']))
        self.assertEqual(response.data['results'][0]['allocation_home'], 'national')
        self.assertEqual(response.data['results'][0]['national'], True)

    def test_filter_allocation_home_unassigned(self):
        self.client.force_authenticate(user=self.admin_user)
        factories.AllocationFactory.create(national=False, associated_site=None)
        response = self.client.get('/rest_api/allocations/?allocation_home=unassigned')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(1, len(response.data['results']))
        self.assertEqual(response.data['results'][0]['allocation_home'], 'unassigned')
    ...
    def test_filter_allocation_home_local(self):
        ...
        factories.AllocationFactory.create(national=False, associated_site=None)
        response = self.client.get('/rest_api/allocations/?allocation_home=%s' % current_site)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(1, len(response.data['results']))
        self.assertEqual(response.data['results'][0]['allocation_home'], current_site)
        self.assertEqual(response.data['results'][0]['national'], False)
        self.assertEqual(response.data['results'][0]['associated_site'], current_site)
