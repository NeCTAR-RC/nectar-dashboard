...
class AllocationTests(base.AllocationAPITest):
    ...
    def test_filter_allocation_home_national(self):
        ...
        self.assertEqual(1, len(response.data['results']))
        self.assertEqual(
            response.data['results'][0]['allocation_home'], 'national'
        )
    ...
    def test_filter_allocation_home_unassigned(self):
        ...
        factories.AllocationFactory.create(
            national=False, associated_site=None
        )
        response = self.client.get(
        ...
        self.assertEqual(1, len(response.data['results']))
        self.assertEqual(
            response.data['results'][0]['allocation_home'], 'unassigned'
        )
    ...
    def test_filter_allocation_home_local(self):
        ...
        response = self.client.get(
            f'/rest_api/allocations/?allocation_home={current_site}'
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(1, len(response.data['results']))
        self.assertEqual(
            response.data['results'][0]['allocation_home'], current_site
        )
        self.assertEqual(response.data['results'][0]['national'], False)
        self.assertEqual(
            response.data['results'][0]['associated_site'], current_site
        )
    ...
    def test_filter_associated_site_name(self):
        ...
        response = self.client.get(
            f'/rest_api/allocations/?associated_site={current_site}'
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(1, len(response.data['results']))
        self.assertEqual(
    ...
    def test_get_allocation_bundle(self):
        ...
        allocation = factories.AllocationFactory.create(
            bundle=silver, create_quotas=False
        )
    ...
    def test_update_allocation(self):
        ...
        response = self.client.patch(
            '/rest_api/allocations/1/', {'use_case': 'test-update'}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_update_national(self):
        ...
        self.client.force_authenticate(user=self.admin_user)
        response = self.client.patch(
            '/rest_api/allocations/1/', {'national': True}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        allocation = models.AllocationRequest.objects.get(
            id=self.allocation.id
    ...
    def test_update_associated_site(self):
        ...
        self.client.force_authenticate(user=self.admin_user)
        response = self.client.patch(
            '/rest_api/allocations/1/', {'associated_site': new_site}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        allocation = models.AllocationRequest.objects.get(
            id=self.allocation.id
    ...
    def test_update_allocation_home_local(self):
        ...
        new_site = 'monash' if current_site != 'monash' else 'qcif'
        response = self.client.patch(
            '/rest_api/allocations/1/', {'allocation_home': new_site}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        allocation = models.AllocationRequest.objects.get(
            id=self.allocation.id
    ...
    def test_update_allocation_home_national(self):
        ...
        current_site = self.allocation.associated_site.name
        response = self.client.patch(
            '/rest_api/allocations/1/', {'allocation_home': 'national'}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        allocation = models.AllocationRequest.objects.get(
            id=self.allocation.id
    ...
    def test_update_allocation_home_unassigned(self):
        self.client.force_authenticate(user=self.admin_user)
        response = self.client.patch(
            '/rest_api/allocations/1/', {'allocation_home': 'unassigned'}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        allocation = models.AllocationRequest.objects.get(
            id=self.allocation.id
    ...
    def test_update_allocation_ardc_none(self):
        self.client.force_authenticate(user=self.admin_user)
        response = self.client.patch(
            '/rest_api/allocations/1/', {'ardc_support': []}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        allocation = models.AllocationRequest.objects.get(
            id=self.allocation.id
    ...
    def test_update_allocation_ardc_two(self):
        self.client.force_authenticate(user=self.admin_user)
        response = self.client.patch(
            '/rest_api/allocations/1/', {'ardc_support': ['cvl', 'BCCVL']}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        allocation = models.AllocationRequest.objects.get(
            id=self.allocation.id
        ...
        response = self.client.get('/rest_api/allocations/1/')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['ardc_support'], ['BCCVL', 'CVL'])

    def test_update_allocation_ardc_unknown(self):
        self.client.force_authenticate(user=self.admin_user)
        response = self.client.patch(
            '/rest_api/allocations/1/', {'ardc_support': ['fish']}
        )
    ...
    def test_update_allocation_facilities_none(self):
        self.client.force_authenticate(user=self.admin_user)
        response = self.client.patch(
            '/rest_api/allocations/1/', {'ncris_facilities': []}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        allocation = models.AllocationRequest.objects.get(
            id=self.allocation.id
    ...
    def test_update_allocation_facilities_two(self):
        self.client.force_authenticate(user=self.admin_user)
        response = self.client.patch(
            '/rest_api/allocations/1/', {'ncris_facilities': ['ala', 'MA']}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        allocation = models.AllocationRequest.objects.get(
            id=self.allocation.id
        ...
        response = self.client.get('/rest_api/allocations/1/')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['ncris_facilities'], ['ALA', 'MA'])

    def test_update_allocation_facilities_unknown(self):
        self.client.force_authenticate(user=self.admin_user)
        response = self.client.patch(
            '/rest_api/allocations/1/', {'ncris_facilities': ['fish']}
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_update_allocation_organizations(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            response.data['supported_organisations'],
            ['https://ror.org/23456789', 'https://ror.org/12345678'],
        )
    ...
    def test_update_allocation_organisations_ambiguous(self):
        models.Organisation.objects.get_or_create(
            ...
            ror_id='https://ror.org/1111',
            country='AU',
        )
        models.Organisation.objects.get_or_create(
            short_name='UZ',
            full_name='Zombie University',
            ror_id='https://ror.org/2222',
            country='AU',
        )
    ...
    def test_update_allocation_negative(self):
        ...
        factories.AllocationFactory.create(contact_email='other@example.com')
        response = self.client.patch(
            '/rest_api/allocations/2/', {'use_case': 'test-update'}
        )
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
    ...
    def test_update_allocation_admin_field(self):
        ...
        factories.AllocationFactory.create(contact_email='other@example.com')
        response = self.client.patch(
            '/rest_api/allocations/2/', {'notes': 'test-notes'}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        self.assertEqual(
            {
                ...
                'concerned_sites': [allocation.associated_site.name],
            },
            response.data,
        )
    ...
    def test_create(self):
        ...
        self.assertEqual(
            models.AllocationRequest.SUBMITTED, response.data['status']
        )
    ...
    def test_create_with_bundle_invalid(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(
            "Bundle 'bogus' does not exist", str(response.data['bundle'][0])
        )
    ...
    def test_create_bad_site(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(
            str(response.data['associated_site'][0]),
    ...
    def test_create_bogus_alloc_home_as_user(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(
            str(response.data['allocation_home'][0]),
            "Site 'Bogus' does not exist",
        )
    ...
    def test_create_alloc_home_conflict(self):
        self.client.force_authenticate(user=self.admin_user)
        data = self._make_data(
            national=False, associated_site='qcif', allocation_home='qcif'
        )
        response = self.client.post('/rest_api/allocations/', data)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(
    ...
    def test_create_duplicate_project_name(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(
            str(response.data['project_name'][0]),
    ...
    def test_create_multiple_usage_types(self):
        ...
        factories.AllocationFactory.create()
        data = self._make_data(
            usage_types=['Other', 'Hosting services to support research']
        )
        response = self.client.post('/rest_api/allocations/', data)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        allocation = models.AllocationRequest.objects.get(
    ...
    def test_create_unknown_usage_type(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(
            str(response.data['usage_types'][0]),
    ...
    def test_approve_invalid_state(self):
        ...
        response = self.client.post(
            f'/rest_api/allocations/{allocation.id}/approve/'
        )
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)

    def test_approve_no_site(self):
        self.client.force_authenticate(user=self.approver_user)
        allocation = factories.AllocationFactory.create(associated_site=None)
        response = self.client.post(
            f'/rest_api/allocations/{allocation.id}/approve/'
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_amend(self):
        ...
        response = self.client.post(
            f'/rest_api/allocations/{allocation.id}/amend/'
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        allocation = models.AllocationRequest.objects.get(id=allocation.id)
        self.assertEqual(
            models.AllocationRequest.UPDATE_PENDING, allocation.status
        )
    ...
    def test_delete(self):
        ...
        self.assertEqual(
            models.AllocationRequest.objects.filter(
                parent_request__id=self.allocation.id
            ).count(),
            0,
        )
        self.client.force_authenticate(user=self.admin_user)
