...
@mock.patch('openstack_auth.utils.is_token_valid', new=lambda x: True)
class AllocationTests(base.AllocationAPITest):
    ...
    def test_update_allocation(self):
        ...
        response = self.client.patch('/rest_api/allocations/1/', {'use_case': 'test-update'})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        allocation = models.AllocationRequest.objects.get(id=self.allocation.id)
        self.assertEqual('test-update', allocation.use_case)

    def test_update_allocation_invalid_state(self):
        self.client.force_authenticate(user=self.user)
        allocation = factories.AllocationFactory.create(status=models.AllocationRequest.APPROVED,
                                                        created_by=self.user.id)
        response = self.client.patch('/rest_api/allocations/%s/' % allocation.id,
                                     {'use_case': 'test-update'})
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)

    def test_update_allocation_read_only_field(self):
        self.client.force_authenticate(user=self.user)
        response = self.client.patch('/rest_api/allocations/1/', {'status': models.AllocationRequest.APPROVED})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_update_allocation_negative(self):
        self.client.force_authenticate(user=self.user)
        factories.AllocationFactory.create(created_by='456')
        response = self.client.patch('/rest_api/allocations/2/', {'use_case': 'test-update'})
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
    ...
    def test_update_allocation_admin_field(self):
        self.client.force_authenticate(user=self.admin_user)
        factories.AllocationFactory.create(created_by='456')
        response = self.client.patch('/rest_api/allocations/2/', {'notes': 'test-notes'})
        self.assertEqual(response.status_code, status.HTTP_200_OK)        
    ...
    def test_create(self):
        ...
        self.assertEqual(models.AllocationRequest.SUBMITTED, response.data['status'])
    ...
    def test_approve(self):
        ...
        response = self.client.post('/rest_api/allocations/%s/approve/' % self.allocation.id)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(models.AllocationRequest.APPROVED, response.data['status'])

    def test_approve_invalid_state(self):
        self.client.force_authenticate(user=self.approver_user)
        allocation = factories.AllocationFactory.create(status=models.AllocationRequest.APPROVED)
        response = self.client.post('/rest_api/allocations/%s/approve/' % allocation.id)
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
    ...
    def test_amend(self):
        self.client.force_authenticate(user=self.user)
        allocation = factories.AllocationFactory.create(created_by=self.user.id,
                                                        status=models.AllocationRequest.APPROVED)
        response = self.client.post('/rest_api/allocations/%s/amend/' % allocation.id)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        allocation = models.AllocationRequest.objects.get(id=allocation.id)
        self.assertEqual(models.AllocationRequest.UPDATE_PENDING, allocation.status)
    ...
    def test_amend_invalid_user(self):
        self.client.force_authenticate(user=self.user)
        allocation = factories.AllocationFactory.create(created_by='456',
                                                        status=models.AllocationRequest.APPROVED)
        response = self.client.post('/rest_api/allocations/%s/amend/' % allocation.id)
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
    ...
    def test_delete(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(models.AllocationRequest.DELETED, response.data['status'])
