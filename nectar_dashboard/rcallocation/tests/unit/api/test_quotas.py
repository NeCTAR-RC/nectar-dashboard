...
class QuotaTests(base.AllocationAPITest):
    ...
    def setUp(self, *args, **kwargs):
        ...
        self.resource = factories.ResourceFactory(
            quota_name='test_resource',
            service_type=self.service_type)
        self.quota_group = factories.QuotaGroupFactory(
            allocation=self.allocation,
            service_type=self.service_type,
            zone=self.zone)
        self.quota = factories.QuotaFactory(group=self.quota_group,
    ...
    def test_update_quota(self):
        self.client.force_authenticate(user=self.admin_user)
        response = self.client.patch('/rest_api/quotas/%s/' % self.quota.id,
                                     {'resource': 4})
    ...
    def test_create_inlimited_managed(self):
        self.client.force_authenticate(user=self.user)
        resource = factories.ResourceFactory(quota_name='test_resource2',
                                             service_type=self.service_type)
        data = {
    ...
    def test_create_inlimited_unmanaged(self):
        self.client.force_authenticate(user=self.user)
        resource = factories.ResourceFactory(quota_name='test_resource2',
                                             service_type=self.service_type)
        data = {
