...
class QuotaTests(base.AllocationAPITest):
    ...
    def setUp(self, *args, **kwargs):
        ...
        self.service_type = factories.ServiceTypeFactory(catalog_name='test_st')
        self.service_type.zones.add(self.zone)
        self.resource = factories.ResourceFactory(quota_name='test_resource',
                                                  service_type=self.service_type)
        self.quota_group = factories.QuotaGroupFactory(allocation=self.allocation,
                                                       service_type=self.service_type,
                                                       zone=self.zone)
        self.quota = factories.QuotaFactory(group=self.quota_group, resource=self.resource)

    def _get_quotas(self, user):
        return models.Quota.objects.filter(group__allocation__created_by=user.id)
    ...
    def test_list_quotas(self):
        ...
        self.assertEqual(self._get_quotas(self.user).count(), len(response.data))
    ...
    def test_list_quotas_admin(self):
        ...
        factories.AllocationFactory.create(created_by='456', create_quotas=True)
        factories.AllocationFactory.create(created_by='XYZ', create_quotas=True)
        response = self.client.get('/rest_api/quotas/')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_update_quota(self):
        self.client.force_authenticate(user=self.admin_user)
        response = self.client.patch('/rest_api/quotas/%s/' % self.quota.id, {'resource': 4})
        self.assertEqual(response.status_code, status.HTTP_405_METHOD_NOT_ALLOWED)

    def test_delete_quota(self):
        self.client.force_authenticate(user=self.user)
        response = self.client.delete('/rest_api/quotas/%s/' % self.quota.id)
    ...
    def test_delete_quota_invalid_state(self):
        ...
        response = self.client.delete('/rest_api/quotas/%s/' % quota.id)
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
