...
def request_allocation(user, model=None, quota_specs=None,
    ...
    if model:
        ...
        current_usages = model.usage_types.filter(enabled=True) \
                                          .values_list('id', flat=True)
        usage_types = [{'id': usage.id,
                        'usage': usage.id in current_usages
                       }
                       for usage in factories.get_active_usage_types()]
    ...
    for usage_type in usage_types:
        form['usage_type-%s' % usage_type['id']] = usage_type['usage']
