...
class BaseAllocationView(UpdateView):
    ...
    def get_formset(self, formset, queryset=None, prefix=None, initial=None, **kwargs):
    ...
    def get_quota_formsets(self):
        ...
        quota_formsets = OrderedDict()#defaultdict(list)
        for service_type in models.ServiceType.objects.all():
            ...
            existing_resources = []
            if self.object:
                existing_quotas = models.Quota.objects.filter(group__allocation=self.object,
            ...
            if self.object:
                existing_groups = self.object.quotas.filter(service_type=service_type)
                if existing_groups:
                    qg_formsets = []
                    for quotagroup in existing_groups:
                        form = GroupForm(instance=quotagroup,
                                         prefix="%s_%s" % (service_type.catalog_name, quotagroup.id),
                                         **group_form_args)
                    ...
                    #quota_formsets.append((service_type, qg_formsets))
                else:
                    form = GroupForm(prefix="%s_a" % service_type.catalog_name, **group_form_args)
                    formset = self.get_formset(
                        QuotaFormSet,
                        queryset=models.Quota.objects.filter(
                            resource__service_type=service_type),
                    ...
                    #quota_formsets.append((service_type, [(form, formset)]))
            else:
                form = GroupForm(prefix="%s_a" % service_type.catalog_name, **group_form_args)
                formset = self.get_formset(
                    QuotaFormSet,
            ...
            exp = re.compile('%s_(?P<prefix>[b-z])-TOTAL_FORMS' % service_type.catalog_name)
            for key in self.request.POST.keys():
                match = exp.search(key)
                if match:
                    prefix = "%s_%s" % (service_type.catalog_name, match.group('prefix'))
