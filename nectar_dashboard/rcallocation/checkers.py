...
def grant_check(checker):
    ...
    if models.Grant.objects.filter(allocation=checker.allocation,
                                   last_year_funded__lt=last_year).count():
        return (EXPIRED_GRANT,
                'One or more of your listed research grants in %s or earlier. '
                'Old grants that are no longer relevant to '
                'allocation request assessment should be removed from the '
                'form.' % (last_year - 1))
...
def output_checks(checker):
    ...
    if models.Publication.objects.filter(allocation=checker.allocation,
        ...
        return (UNSPECIFIED_OUTPUT,
                'One or more of the Publications / Outputs listed on the '
                'form needs to be reentered with a category and (when '
                'available) a DOI.  When you have done this, please delete '
                'the old entry.')
    if models.Publication.objects.filter(allocation=checker.allocation,
                                         output_type=JOURNAL,
                                         crossref_metadata=None).count():
        return (NO_CROSSREF,
