...
(function($) {
    ...
    function create_form_row(formset, opts) {
        ...
        var new_row = $('input[id=' + new_row_id + ']').closest('tr');
        apply_publication_handlers(new_row);
        total_rows += 1;
        var total_forms_input = $('#id_' + opts.prefix + '-TOTAL_FORMS');
        total_forms_input.val(total_rows);
    };
    ...
    function create_row(prefix, row_index, opts){
        ...
        new_row += "</div>";
        //doi prompts
        new_row += create_field_div(opts, 'prompts', row_index, 'hidden');
        new_row += "<div>";
        new_row += "All recently published books and papers should have a ";
        new_row += "Digital Object Identifier (DOI) issued by the publisher. ";
        new_row += "We need you to find and provide us with that DOI, ";
        new_row += "if it exists.";
        new_row += "</div>";
        ...
        new_row += "</div>";
        // details
        new_row += create_field_div(opts, 'details', row_index, 'hidden');
        new_row += create_input_field_label(opts, 'details', 'Publication details (from crossref)', row_index, false, '');
        new_row += "<div id='id_" + opts.prefix + "-" + row_index + "-details'>";
        new_row += "</div>";
        new_row += "</div>";
        // closed details div
        new_row += "</div>"
...
function apply_publication_handlers(tr) {
    tr.off('change', 'select[id$=-output_type]');
    tr.on('change', 'select[id$=-output_type]', function(e) {
        var output_type = $(this).val();
        var current_tr = $(this).closest('tr');
        var prompts_group = current_tr.find('div[id$=-prompts-group]');
        var doi_group = current_tr.find('div[id$=-doi-group]');
        var doi_input = current_tr.find('input[id$=-doi]');
        var details_group = current_tr.find('div[id$=-details-group]');
        var meta_input = current_tr.find('input[id$=-crossref_metadata]');
        var pub_group = current_tr.find('div[id$=-publication-group]');
        var pub_input = current_tr.find('input[id$=-publication]');
    ...
    tr.on('click', 'button[name=have-doi]', function(e) {
        var current_tr = $(this).closest('tr');
        current_tr.find('div[id$=-prompts-group]').hide();
...
function render_crossref_metadata(json) {
    try {
        ...
        return "Title: " + data.message['title'] +
            ", Author(s): " + format_authors(data.message['author']) +
            ", Publication: " + data.message['container-title'] +
...
function accept_doi(e) {
    ...
    var details_div = $('#id_publications-' + row_no + '-details');
    var metadata = $('#doi-crossref').val();
    crossref_input.val(metadata);
    details_div.text(render_crossref_metadata(metadata));
    $('#id_publications-' + row_no + '-publication-group').hide();
    $('#id_publications-' + row_no + '-details-group').show();
};
...
function reject_doi(e) {
    ...
    var details_div = $('#id_publications-' + row_no + '-details');
    crossref_input.val('');
    details_div.text("No information available for DOI " + doi);
    $('#id_publications-' + row_no + '-publication-group').show();
    $('#id_publications-' + row_no + '-details-group').hide();
};
