...
class FreshdeskNotifier(NotifierBase):
    ...
    def send_email(self, email, subject, body) -> str:
        ...
        ticket_id = ticket.id
        LOG.info(
            f"Allocation {self.allocation.id}: Created outbound email "
    ...
    def create_ticket(self, user, approver, subject, body, note):
        ...
        ticket_id = ticket.id
        LOG.info(
            f"Allocation {self.allocation.id}: Created ticket "
    ...
    def update_ticket(self, ticket_id, text, cc_emails=[]):
        ...
        LOG.info(
            f"Allocation {self.allocation.id}: Sent reply to "
            f"ticket {ticket_id}"
        )
    ...
    def update_ticket_requester(self, ticket_id, owner):
        ...
        LOG.debug(
            f"Allocation {self.allocation.id}: Set ticket requester "
    ...
    def add_note_to_ticket(self, ticket_id, text):
        ...
        LOG.info(
            f"Allocation {self.allocation.id}: Added private note "
