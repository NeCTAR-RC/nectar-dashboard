...
def partition_active_allocations():
    ...
    active_allocations = models.AllocationRequest.objects.filter(parent_request__isnull=True).prefetch_related(
        Prefetch('quotas', queryset=models.QuotaGroup.objects.filter(service_type='compute', zone='nectar')),
...
def traverse_allocations_tree(allocations_tree, node_parent, recursion_depth):
    ...
    for node_name in allocations_tree.keys():
        ...
        if recursion_depth < MAX_RECURSION_DEPTH:
            traverse_allocations_tree(allocations_subtree, node_children, recursion_depth + 1)
        else:
            for allocation_summary in allocations_subtree:
                allocation_items = create_allocation_tree_leaf_node(allocation_summary)
...
def apply_partitioned_quotas(allocation, allocation_summary, percentage):
    ...
    if allocation.quotas.all():
        for quota in allocation.quotas.all()[0].quota_cache:
            if quota.resource_id == 81: # TODO
                instance_quota = quota.quota
            elif quota.resource_id == 75: # TODO
...
def summary(allocation, code):
    ...
    allocation_summary['institution'] = institution_from_email(allocation.contact_email)
