...
class QuotaSanityContext(object):
    ...
    def get(self, quota_name, zone='nectar'):
        quotas = filter(lambda q: q['name'] == quota_name
                        and q['zone'] == zone, self.all_quotas.values())
        if len(quotas) >= 1:
    ...
    def get_all(self, quota_name):
        return filter(lambda q: q['name'] == quota_name
                      and q['value'] > 0, self.all_quotas.values())
