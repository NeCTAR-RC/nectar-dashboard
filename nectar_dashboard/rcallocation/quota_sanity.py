...
def zone_to_home(zone):
    for home, zones in settings.ALLOCATION_HOME_ZONE_MAPPINGS.items():
        if zone in zones:
            return home
    raise Exception('no home mapping for resource zone %s' % zone)
...
def cinder_local_check(context):
    ...
    if is_node_local(alloc_home):
        for q in context.get_all('volume.gigabytes'):
            if q['value'] > 0 and zone_to_home(q['zone']) != alloc_home:
