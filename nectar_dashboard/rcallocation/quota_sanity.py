...
def approver_authority_checks(context):
    if context.approver is None:
        return
    try:
        approver = models.Approver.objects.get(email=context.approver.username)
    except models.Approver.DoesNotExist:
        LOG.warning("No Approver object found for '%s'",
                    context.approver.username)
...
STD_CHECKS = [instance_vcpu_check,
              ...
              manila_local_check,
              neutron_checks,
              approver_authority_checks]


class QuotaSanityContext(object):

    def __init__(self, form=None, requested=True, approver=None,
