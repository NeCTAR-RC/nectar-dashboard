...
class AllocationApproveForm(forms.ModelForm, base_forms.QuotaMixin):
    ...
    class Meta:
        ...
        widgets = {
            ...
            'national': forms.CheckboxInput(
                attrs={'class': 'col-md-6', 'style': 'height:20px; width:20px'}
            ),
        }
    ...
    def __init__(self, **kwargs):
        ...
        self.fields['associated_site'].help_text = (
            "The Approver will normally set the Associated Site to their own "
            "node."
        )
        self.fields['associated_site'].queryset = models.Site.objects.filter(
            enabled=True
        )
