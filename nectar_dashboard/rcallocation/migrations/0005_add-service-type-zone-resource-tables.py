...
def populate_zone_and_resources(apps, schema_editor):
    ...
    for quota_type, azs in getattr(settings, 'ALLOCATION_QUOTA_AZ_CHOICES', {}).items():
        service_type_name = quota_type.split('.')[0]
        service_type, created = ServiceType.objects.get_or_create(catalog_name=service_type_name,
                                                         name=service_type_name)
        for name, display_name in azs:
            zone, created = Zone.objects.get_or_create(name=name, display_name=display_name)
    ...
    for quota_type in getattr(settings, 'ALLOCATION_QUOTA_TYPES', ()):
        ...
        Resource.objects.get_or_create(name=name, service_type=service_type, quota_name=quota_name, unit=unit)


class Migration(migrations.Migration):

    dependencies = [
        ('rcallocation', '0004_minor_field_changes'),
    ]
    ...
    operations = [
        ...
        migrations.CreateModel(
            name='ServiceType',
            fields=[
                ('catalog_name', models.CharField(max_length=32, serialize=False, primary_key=True)),
                ('name', models.CharField(max_length=64)),
            ],
        ),
        migrations.CreateModel(
