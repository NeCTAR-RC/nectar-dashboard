...
def migrate_compute_quota(apps, schema_editor):
    ...
    zone, created = Zone.objects.get_or_create(name='nectar', defaults={'display_name': 'Nectar'})
    st, created = ServiceType.objects.get_or_create(catalog_name='compute', defaults={'name': 'Compute Service'})
    st.zones.add(zone)
    core_resource, created = Resource.objects.get_or_create(quota_name='cores', service_type=st, defaults={'unit': 'VCPUs', 'name': 'Virtual Cores', 'service_type': st})

    ram_resource, created = Resource.objects.get_or_create(quota_name='ram', service_type=st, defaults={'unit': 'GB', 'name': 'RAM', 'service_type': st})

    instance_resource, created = Resource.objects.get_or_create(quota_name='instances', service_type=st, defaults={'unit': 'servers', 'name': 'Instances', 'service_type': st})

    for allocation in AllocationRequest.objects.all():
        Quota.objects.create(allocation=allocation,
                             zone=zone,
