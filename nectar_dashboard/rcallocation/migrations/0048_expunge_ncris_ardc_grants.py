    ...
    def expunge(apps, schema_editor):
        ...
        for grant in Grant.objects.all():
            ...
            if allocation.nectar_support or allocation.ncris_support:
                grant.delete()
                LOG.info("Deleting grant %s for allocation %s",
                         grant.id, allocation.id)
            else:
                errors += 1
                LOG.error("You haven't set the support field(s) for"
                          " allocation %s", allocation.id)

        if errors > 0:
            raise Exception('Abandoning migration: see logs for errors')
