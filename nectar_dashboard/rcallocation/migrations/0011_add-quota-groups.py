...
def create_dummy_group(apps, schema_editor):
     ...
     try:
          ...
          return
     zone = Zone.objects.create(name=DUMMY_SERVICE_TYPE, display_name=DUMMY_SERVICE_TYPE)
     st = ServiceType.objects.create(name=DUMMY_SERVICE_TYPE, catalog_name=DUMMY_SERVICE_TYPE)

     QuotaGroup.objects.create(id=1, allocation=allocation, service_type=st, zone=zone)
...
def convert_quotas(apps, schema_editor):
    ...
    for quota in Quota.objects.all():
        qg, created = QuotaGroup.objects.get_or_create(allocation=quota.allocation,
...
class Migration(migrations.Migration):
     ...
     operations = [
          ...
          migrations.AddField(
               ...
               name='allocation',
               field=models.ForeignKey(to='rcallocation.AllocationRequest'),
          ),
          migrations.AddField(
               model_name='quotagroup',
               name='service_type',
